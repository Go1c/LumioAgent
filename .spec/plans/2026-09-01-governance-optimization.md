...
**closeout-gate.mjs**（新工具，`plugin/tools/`）：输入 diff，输出三态 + 命中理由——`快速豁免`（纯文档 / 纯注释 / 纯配置数据 / 有效行 < 20 / revert）、`快审`、`深审`；红线面路径（`rules/`、hooks、鉴权面）一票升级为至少快审。**白名单随之收窄到机器可判子集**：原白名单中「机械套用既有模式」「生成物随源更新」机器判不了，不再享豁免、归入快审。把 dispatch.md 已有的「判定须机器可判」从散文变成命令。
