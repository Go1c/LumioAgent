...
- **Claude Code 通过 `CLAUDE.md` 的 `@import` 把具体规则文件强制载入每次会话上下文**（始终在场，不走渐进式披露）；新增规则文件要在 `CLAUDE.md` 补一行 `@import`。软链接 `.claude/rules -> ../.spec/rules` 指向同一套源。
- Codex 通过根 `AGENTS.md` 指针读取这些规则（无 @import，仍靠自觉读）。
...
- **加载层（已强制）**：Claude 通过 `CLAUDE.md` 的 `@import` 把规则**强制载入上下文**，每次会话一上来就在场——比「自觉去读」强一档，规则不会被忽略 / 遗漏。
- **遵守层（仍是约定）**：载入后「遵不遵守」仍靠主 loop 和子 Agent 自觉，宿主不替我们机制级拦截（符合本项目「约定为主、runtime 暂缓」的定位）。
