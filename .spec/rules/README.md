...
本目录存放 **Agent 护栏 / 禁令（guardrails）**：硬性的“**禁止** Agent 访问 / 搜索 / 修改 / 提交某物”这类通用约束。

这里的规则属于 `.spec/`，因此是 LumioAgent 的权威规范组成部分。它适合存放稳定的、跨工具、跨 Agent、**与具体任务无关**的禁令：这些规则比 `.spec/AGENTS.md` 更具体，但又不属于某一个 Skill 或某一个 SubAgent。

判据很简单——**rules 回答“禁止碰什么”，不回答“怎么做”。**

- ✅ 属于 rules：不准 push 到 `main`、不准提交 `secrets/`、不准修改 `vendor/`、不准搜索 / 外发某类数据。
- ❌ 不属于 rules（这些是**流程 / 规范**，放 `knowledge/standards/`）：怎么开分支、怎么写提交信息、怎么合并、注释规范、要不要走 TDD、怎么验收。
...
- 全局身份、调度策略、Agent 协作红线放在 `.spec/AGENTS.md`。
