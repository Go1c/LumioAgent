...
When implementation is complete, integrate the work in three steps.

### 1. Verify

Run the project's verification gate (test suite / 收口门槛 command) and read the output. **Failing verification blocks integration** — report the failures and fix first. Never merge broken work.
...
- **Only after a successful merge or an explicitly confirmed discard.** Keep the worktree alive whenever work continues there (PR iteration, kept branch).
- **Only clean up worktrees this workflow created** (under `.worktrees/` or `worktrees/`). Harness-owned workspaces: use the platform's exit tool if one exists; otherwise leave them in place — removing them causes phantom state.
- **Discard is destructive.** Before deleting a branch or its commits, list exactly what will be lost and get explicit user confirmation.
- `cd` to the main repo root first — `git worktree remove` fails when run from inside the worktree being removed. Remove the worktree before deleting the branch (the worktree pins it):
