...
- **规范主体使用中文**（`.spec/` 下全部文档）；例外：根 `CLAUDE.md`（宿主入口惯例）与 `skills/` 下允许英文技能文档（中英以该技能既有语言为准，不混写）。若改用其他语言，需全仓一致——注意 LumioAgent 插件的 spec-lint 内置中文 status 枚举，换语言需同步上游。
...
- 生成物不得手改，只能经生成源与生成命令更新，并与生成源一起提交（红线由 LumioAgent 插件每次会话注入）。
